...
- Evaluation suite (phase 8):
  ...
  - `eval/metrics.py`: hit@k, MRR, context precision (rank-weighted judged relevance), context recall, faithfulness, answer relevance, correct and false refusal rates, latency p50/p95 and cost per query.
  - `eval/run_eval.py`: scores dense, hybrid, hybrid + reranking and the full pipeline over the same questions with the cache off, and writes `summary.md`, `summary.csv` and `results.csv`. Results are checkpointed per question so an interrupted run resumes; model fallback is disabled so configurations are compared on the same models, and questions slowed by rate limiting are excluded from the latency figures.
  - Results on the sample corpus (README has the tables): dense retrieval alone answers everything, BM25 and reranking lose paraphrased questions (down to 50-60% hit@5 on those), and query rewriting plus self-correction restore 100% at about 4x the latency. No configuration answered a question the documents don't cover.
- Reranking no longer hands the answer model an empty context: at least `RERANK_MIN_KEEP` (3) passages survive the score threshold. The evaluation found the threshold dropping every passage on some questions, which turned answerable questions into refusals (hybrid + reranking: 15% false refusals before, 9% after; the full pipeline 8% before, 4% after).
