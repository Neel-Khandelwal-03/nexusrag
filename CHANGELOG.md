...
- Semantic cache (phase 7):
  - After routing, the standalone question is embedded and matched against cached answers in the same knowledge base. A hit returns the stored answer with its citations and skips retrieval, grading and generation. On the sample corpus that took ~1.5–2 s and ~$0.0004, against ~15 s and ~$0.003–0.006 for the full pipeline.
  - Reuse requires the same knowledge base version, the same settings fingerprint (route, search scope, style, retrieval switches, self-correction, models), cosine similarity ≥ `CACHE_SIMILARITY_THRESHOLD`, and identical numbers and codes (Q2, 2026, X1, CH-400).
  - The default threshold is 0.96, calibrated on gemini-embedding-2: paraphrases scored 0.952–0.995 and different questions up to 0.938. "Q2 2026" vs "Q1 2026" scored 0.947, which only the key-term guard catches.
  - Stored in the shared SQLite database (schema version 2). Ingestion and deletion invalidate the knowledge base's entries in the same transaction. Only cited, grounded, non-refusal answers are stored, with up to `CACHE_MAX_ENTRIES` per knowledge base (least recently used evicted).
  - The UI gains a cache switch in the settings panel, a *Check semantic cache* pipeline step (similarity, matched question, age, reuse count, or why it missed), a note on cached answers, the hit rate and entry count in `/stats`, and a `/clear-cache` command.
  - `RAGService.ask(use_cache=...)` (the evaluation turns it off), `RAGService.clear_cache()`, and `Answer.cached` / `Answer.cache_similarity`.
  - A per-request query-embedding memo lets the cache check and retrieval share one embedding of the question, so a miss costs no extra embedding call.
- Only just-uploaded documents restrict the search: a question naming another document ("the Aurora X1's price") still searches everything, since the answer may live in a different file.
